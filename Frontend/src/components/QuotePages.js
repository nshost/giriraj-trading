  ...
  const handleGetFinalQuotes = async () => {
    try {
      ...
      if (!userEmail) {
        ...
        toast.error('Please log in first to get the final products');
        return;
      }
      ...
      const requestData = {
        userEmail,
        products: selectedProducts.map(product => product.name), //products should be an array of product names
      };
      ...
      if (response.ok) {
        ...
        toast.error('Error sending products. Please try again later.');
      }
    } catch (error) {
